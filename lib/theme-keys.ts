// 店面主題「合法值清單」的唯一正本：Hero 四種版型、首頁 11 個 section、對齊三值等。
//
// 這些清單以前在公開頁 _theme.ts、編輯器與設定頁的存檔 actions、editor-workspace、
// AI 助手 lib/ai-theme-patch 各自手抄一份，多一種 hero 版型或多一個 section 得每處一起補，
// 漏一處就是「編輯器能選、存檔被濾掉」這種後台看不出來的錯。現在全部收在這裡，型別也從
// 這裡導出，其他地方只 import；要加值只改這一檔。
//
