...
export const SECTION_STYLE_ENUMS = {
  ...
  // 這一欄只管上下（object-position 直向那一半），左右在下一欄。
  mediaFocus: ["auto", "top", "bottom"],
  // 照片左右取景（auto 置中 / left 保留左緣 / right 保留右緣），跟上一欄各管一軸、可以疊。
  // 上一欄剛開的時候只給上下，理由是「卡片圖框永遠比照片窄邊裁長邊、直式照片被裁的是上下」
  // ——那是每段圖框比例還寫死的時候。「照片比例」開放之後這個前提就不成立了：照片牆預設
  // 正方形、選物可以切成直式 3:4，商家把橫式的店面外觀、桌上一排商品放進去，被切掉的是
  // 左右兩側，主體站在畫面一邊的照片從正中間裁下去主體只剩半個，上一欄怎麼按都沒反應
  // （它只動直向）。hero 的 split 版型上一輪補過同一格（heroImageFocusX），這裡是同一個
  // 問題在卡片圖框上的版本。兩欄各管一軸：留上緣 + 留左緣 = 保留左上角。
  mediaFocusX: ["auto", "left", "right"],
  ...
  // 卡片外觀（none 原樣 / panel 淡底色的面板 / outline 一圈細框 / both 底＋框），只套這一段的卡片。
  ...
  // both（底＋框）是後補的第四檔：前三檔把底與框當成二選一，商家按了面板就沒有線、
  // 按了框就沒有底。實際上最常見的商品卡（一塊淺色板子加一圈比它深一點的線）兩個都要
  // ——只有底的卡片在同色系的段落底上邊緣是糊的，只有框的卡片在照片很滿的格子牆裡那條
  // 線會被照片吃掉；兩個一起上，底負責把卡片從背景抬起來、線負責收住邊。
  // 後面那五格（框線粗細 / 深淺 / 樣式、底色深淺）在這一檔全部可用，兩邊各調各的。
  cardSurface: ["none", "panel", "outline", "both"],
  ...
  // 情況這個綁法是對的；但那是「一個決定同時管兩件事」，商家想調的常常只有其中一件。
  // 兩個方向都卡到：走報紙、型錄那種硬邊排版的店，卡片要的是完全的直角，但直角只能靠把
  // 內距收到最緊才最接近（8px 還是圓的），而收緊內距同時把卡片裡的留白也擠掉了；反過來
  // 想要更圓的卡（一列一張的大卡、慢讀那種軟調的段落），只能選放寬內距換到 22px，卡片
  // 跟著整個變胖、一個螢幕看得到的品項變少。
  // 其他格也到不了：段落自己的「圓角」畫的是整段外圍那一個大框，分不到每張卡身上；
  // 「照片圓角」（mediaRadius）改的是卡片裡那張照片，卡片外框的四角不會跟著動。
  // 直角給 0、更圓給 26px（比放寬內距那檔的 22px 再圓一點，但不到藥丸那種弧度——卡片是
  // 方形的容器，圓過頭四個角會把照片與品名的邊切掉）。
  // 沒設（或選「跟內距走」）就沒 attribute，維持原本跟著內距等比走的那組值。
  cardRadius: ["square", "auto", "round"],
  // 卡片陰影（none 照原本的沒有陰影 / soft 輕 / strong 明顯），跟圓角、內距一樣設了卡片底
  // 或框之後才有意義。上一格補的是四個角，這一格補的是卡片浮不浮起來——設了底或框之後，
  // 卡片靜止時是完全平的，只有滑鼠移上去才浮 6px 加一層陰影（見 layout.tsx 那組 hover 規則）。
  // 缺這格會怎樣：客人第一眼看到的是一整片平貼在段落底色上的方塊，而「面板」那檔的底色是
  // currentColor 的 6%（很淡），淺底的店根本看不出卡片跟背景的界線，等於設了外觀還是一堆
  // 散字；手機上更明顯——手機沒有滑過這件事，那層 hover 陰影客人一輩子看不到。反過來想要
  // 完全平的硬邊卡（報紙、型錄那種）現在是預設，不用動。
  // 商家原本沒有一格動得到：段落自己的「陰影」畫的是整段外圍那一個大框，分不到每張卡身上；
  // 「卡片外觀」給的是底或框（兩種都只畫界線，不畫浮起）；「滑過卡片」調的是滑上去之後
  // 做什麼，靜止時那張卡是平的這件事它管不到。
  // 輕用 --sproutly-elev-2（站上照片本來那圈陰影的同一個值，設了卡片外觀之後照片那圈會被
  // 收掉，等於把它搬到卡片外框上）、明顯用 --sproutly-elev-3（滑過時卡片浮起的那一層）。
  // 不給更重的 elev-4：那是 hero 那種整塊大面積在用的，卡片吃下去四周會糊成一圈灰。
  // 滑過時照原本的走（預設檔浮起換成 elev-3，設「輕」的卡剛好是滑上去更浮一階）；商家把
  // 「滑過卡片」按成輕微的那一檔原本會把陰影收成 none，那是為了平的卡片寫的，設了這格之後
  // 要留住靜止的那層，不然滑上去反而變平——layout.tsx 那邊補了兩條蓋回來。
  // 沒設（或選「無」）就沒 attribute、整組規則不存在，既有店家的卡片一個像素都不動。
  cardShadow: ["none", "soft", "strong"],
  // 卡片框線粗細（normal 照原本的 1px / medium 2px / thick 3px），只在「卡片外觀」設成
  // 一圈細框或底＋框那兩檔才有東西可調（面板那檔畫的是底色，沒有線可以加粗）。上面幾格把卡片的
  // 內距、四個角、浮不浮起來都補過了，那條線本身從頭到尾寫死是 1px 加 currentColor 的 16%
  // ——一條又細又淡的線，那是刻意挑的安靜值，配站上預設那種留白多的雜誌排版剛好。
  // 但商家真的按下「外框」通常就是嫌卡片沒有邊界：3、4 欄的小卡排滿一整片時，1px 的
  // 淡線在遠一點的距離上等於不存在，客人看到的還是一堆散字，商家會以為這一格根本沒生效
  //（尤其是把底色設成灰的段落，線跟底幾乎同色）。反方向也有——走報紙、型錄那種硬邊排版
  // 的店（上一格剛給了直角），框就是那個版面的主角，要的是看得見的實線。
  // 商家原本沒有一格動得到：段落自己的「外框」畫的是整段外圍那一個大框，分不到每張卡
  // 身上；「分隔線粗細」動的是段落上下那條橫線；把整段文字色改深會連整段的字一起換掉。
  // 三檔給 1 / 2 / 3px，不跟分隔線那組（1 / 2 / 4px）走：卡片的框是四邊圍一圈、一列還有
  // 三四張並排，4px 乘上四邊再乘上一整列，整片看起來是黑格子牆而不是商品；線也不給比
  // 預設更細的一檔，1px 已經是螢幕上畫得出來的最細（非整數會被瀏覽器抹成灰邊）。
  // 沒設（或選預設那檔）就沒 attribute，線維持原本的 1px。
  cardBorderWeight: ["normal", "medium", "thick"],
  // 卡片框線深淺（normal 照原本的 currentColor 16% / soft 8% / strong 34%），跟粗細同一個
  // 條件——設成一圈細框或底＋框才有意義。上一格解的是線有多粗，這格解的是線有多明顯，兩件
  // 事要分開給：把線加粗到 3px 卻還是 16% 的淡色，看起來是一條糊掉的灰帶而不是框（粗的
  // 線更需要色夠實才收得住邊）；反過來想要一條看得見但不搶戲的細框，只加粗會連重量一起
  // 加上去。商家把兩格配起來才調得出他要的那條線。
  // soft 那檔是給另一種情況的：照片本身很滿的段落（照片牆、大圖商品卡），16% 的框壓在
  // 照片邊上會跟照片打架，商家要的只是一條「隱約分得出這是一張卡」的線。
  // 顏色照樣從該段的文字色算（currentColor 的百分比），跟底色、框線本來的口徑一致——
  // 深底淺字的段落自動變成淺色的框，商家不用再挑一次顏色，也挑不出跟底色同色的看不見值。
  // 不給主色那一檔：這組規則寫在 layout.tsx 的全站 CSS 裡，該段算好的主色只在 page.tsx
  // 那層的 inline style 上，到不了卡片這層（分隔線、側邊色條的 accent 檔是在那裡算的）。
  // 沒設（或選預設那檔）就沒 attribute，線維持原本的 16%。
  cardBorderTone: ["normal", "soft", "strong"],
  // 卡片框線樣式（solid 照原本的實線 / dashed 虛線 / dotted 點線），跟粗細、深淺同一個
  // 條件——設成一圈細框或底＋框才有意義。前兩格解的是那條線有多粗、有多明顯，這一格解的是
  // 那條線「是不是一條實線」，是另一件事：粗細與深淺再怎麼配，畫出來永遠是一圈規規矩矩的
  // 實線框，那條線在版面上的語氣是「這是一張已經定案的卡」。虛線與點線講的是相反的話——
  // 預告、缺貨、名額已滿、還沒上架的那種佔位卡，站上目前只能靠文字寫「即將推出」，框本身
  // 看起來跟正在賣的商品一模一樣；點線框則是拼貼、手作、剪貼簿那類店的排版語言，一圈實線
  // 對他們來說太工整。
  // 商家原本一格都到不了：段落自己的「外框樣式」畫的是整段外圍那一個大框，分不到每張卡
  // 身上；「分隔線樣式」動的是段落上下那條橫線。兩個都給了虛線與點線，唯獨卡片這一圈沒有。
  // 只給三檔、跟分隔線與外框那兩組同一套字（solid / dashed / dotted），不另外發明 double、
  // groove 這些：一列排三四張卡、每張四邊圍一圈，雙線與立體邊在小尺寸上糊成一條髒邊，而且
  // 站上另外兩組線都是這三檔，卡片這格用不一樣的字商家會以為是兩套東西。
  // 跟粗細疊起來才好用：1px 的虛線在遠一點的距離上斷斷續續幾乎看不出是虛的，商家想要的
  // 「這張卡還沒定案」得配 2px 以上才讀得出來——所以三格都留著各自獨立，不合成一格。
  // 沒設（或選預設那檔）就沒 attribute，線維持原本的實線。
  cardBorderStyle: ["solid", "dashed", "dotted"],
  // 卡片底色深淺（normal 照原本的 6% / soft 淡 / strong 明顯），跟框線那三格相反的條件
  // ——卡片外觀設成「一塊底色面板」或「底＋框」才有意義（只有一圈細框那檔畫的是線，沒有底可以
  // 調深淺）。框線那邊已經給了粗細、深淺、樣式三格，面板這邊從頭到尾只有一個寫死的
  // currentColor 6%，商家按下面板之後看到什麼就是什麼，一格都動不了。
  // 6% 是照站上預設那種奶油白底、深色字的段落挑的，換成商家自己的配色就常常不對：整段
  // 底色已經是淺灰、米色那類的店，卡片那層 6% 疊上去幾乎跟背景同一色，商家按了面板卻
  // 看不出卡片在哪，多半會以為這一格壞了；反方向是深底淺字的段落，同樣的 6% 換成淺色疊
  // 在深底上，對比比淺底那邊強得多、一片卡片牆會亮得比照片還搶。
  // 淡那檔給的是「隱約分得出這是一張卡」（照片很滿的商品卡、照片牆那種，底色重一點就會
  // 跟照片打架）；明顯那檔給的是「卡片是一塊實實在在的板子」（字多、照片少的段落，例如
  // 常見問題、客人的話，底夠實才撐得住整段文字）。
  // 顏色照樣走 currentColor 的百分比，跟底色、框線、底紋同一個口徑——深底淺字的段落自動
  // 變成淺色面板，商家不用再挑一次顏色，也挑不出跟底色同色的看不見值。
  // 沒設（或選預設那檔）就沒 attribute，底維持原本的 6%。
  cardPanelTone: ["normal", "soft", "strong"],
...
export const SECTION_STYLE_NEUTRAL_VALUES = {
  ...
  cardRadius: "auto",
  cardShadow: "none",
  cardBorderWeight: "normal",
  cardBorderTone: "normal",
  cardBorderStyle: "solid",
  cardPanelTone: "normal",
  cardLayout: "stack",
  cardMediaWidth: "auto",
  mobileColumns: "auto",
  cardTitleLines: "auto",
