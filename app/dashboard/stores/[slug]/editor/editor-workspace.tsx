...
// SECTION_LINE_STYLE_OPTIONS：實線／虛線／點線的分隔線／底線／外框／色條／卡片框線五格，再收五格。
// SECTION_TEXT_SCALE_OPTIONS／SECTION_TEXT_TONE_OPTIONS：中檔寫「預設」（不是「跟預設」）的
// 內文／標題大小兩格、標題／卡片品名用色兩格，再收四格。
import { applySectionStylePatch, SECTION_BG_STRENGTH_OPTIONS, SECTION_BODY_TRACKING_EM, SECTION_BORDER_RADIUS_PX, SECTION_GAP_OPTIONS, SECTION_HEADING_SCALE, SECTION_LEADING_OPTIONS, SECTION_LINE_STYLE_OPTIONS, SECTION_LINE_TONE_OPTIONS, SECTION_LINE_WEIGHT_OPTIONS, SECTION_MIN_HEIGHT_VH, SECTION_SCALE_OPTIONS, SECTION_TEXT_SCALE_OPTIONS, SECTION_TEXT_TONE_OPTIONS, SECTION_TRACKING_OPTIONS, SECTION_WEIGHT_OPTIONS, type SectionStylePatch } from "@/lib/section-style-schema";
...
type EditorTheme = {
  ...
  layout: {
    ...
    heroSubtitleAlign: LayoutChoice<"heroSubtitleAlign">;
    ...
    heroCtaCase: LayoutChoice<"heroCtaCase">;
    heroCtaWeight: LayoutChoice<"heroCtaWeight">;
    ...
    heroSplitRatio: LayoutChoice<"heroSplitRatio">;
    heroImageFocus: LayoutChoice<"heroImageFocus">;
    heroImageFocusX: AlignX;
    heroSplitImageFit: LayoutChoice<"heroSplitImageFit">;
    heroSplitImageAspect: LayoutChoice<"heroSplitImageAspect">;
    heroSplitTextAlign: LayoutChoice<"heroSplitTextAlign">;
    heroSplitTextAlignX: AlignX;
    heroSplitTextPadding: LayoutChoice<"heroSplitTextPadding">;
    heroSplitMobilePadY: LayoutChoice<"heroSplitMobilePadY">;
    heroSplitGap: LayoutChoice<"heroSplitGap">;
    heroSplitMobileOrder: LayoutChoice<"heroSplitMobileOrder">;
    heroSplitHeight: LayoutChoice<"heroSplitHeight">;
    heroSplitTextBg: string | null;
    heroSplitImageBg: string | null;
    heroSplitDivider: LayoutChoice<"heroSplitDivider">;
    heroSplitDividerTone: LayoutChoice<"heroSplitDividerTone">;
    heroMagazineRuleWeight: LayoutChoice<"heroMagazineRuleWeight">;
    heroMagazineRuleTone: LayoutChoice<"heroMagazineRuleTone">;
    heroMagazineGap: LayoutChoice<"heroMagazineGap">;
    heroMagazineGapMobile: LayoutChoice<"heroMagazineGapMobile">;
    heroMagazineTextWidth: LayoutChoice<"heroMagazineTextWidth">;
    heroMagazineRuleWidth: LayoutChoice<"heroMagazineRuleWidth">;
    heroMagazinePadX: LayoutChoice<"heroMagazinePadX">;
    heroMagazinePadY: LayoutChoice<"heroMagazinePadY">;
    heroMagazineSubtitleWidth: LayoutChoice<"heroMagazineSubtitleWidth">;
    heroMagazineTextGap: LayoutChoice<"heroMagazineTextGap">;
    heroMagazineBg: string | null;
    heroMinimalWidth: LayoutChoice<"heroMinimalWidth">;
    heroMinimalPadding: LayoutChoice<"heroMinimalPadding">;
    heroMinimalPaddingMobile: LayoutChoice<"heroMinimalPaddingMobile">;
    heroMinimalPadX: LayoutChoice<"heroMinimalPadX">;
    heroMinimalPadXMobile: LayoutChoice<"heroMinimalPadXMobile">;
    heroMinimalRule: LayoutChoice<"heroMinimalRule">;
    heroMinimalRuleColor: string | null;
    heroMinimalRuleWeight: LayoutChoice<"heroMinimalRuleWeight">;
    heroMinimalAlign: AlignX;
    heroMinimalBg: string | null;
    heroMinimalGap: LayoutChoice<"heroMinimalGap">;
    heroTextBg: string | null;
    heroTextPadding: LayoutChoice<"heroTextPadding">;
    heroTextWidth: LayoutChoice<"heroTextWidth">;
    heroTextAlignX: AlignX;
    heroTextGap: LayoutChoice<"heroTextGap">;
    heroImageMaxHeight: LayoutChoice<"heroImageMaxHeight">;
    heroFullImageFit: LayoutChoice<"heroFullImageFit">;
    heroFullImageBg: string | null;
    heroImageBounds: HeroImageBounds | null;
    heroHeight: LayoutChoice<"heroHeight">;
    heroHeightMobile: LayoutChoice<"heroHeightMobile">;
    heroFullTextAlignY: LayoutChoice<"heroFullTextAlignY">;
    fontScale: number;
    sectionPaddingScale: LayoutChoice<"sectionPaddingScale">;
    buttonRadius: LayoutChoice<"buttonRadius">;
    footerBg: string | null;
    footerText: string | null;
    featuredCount: number;
    featuredColumns: LayoutColumns<"featuredColumns">;
    collectionsColumns: LayoutColumns<"collectionsColumns">;
    testimonialsColumns: LayoutColumns<"testimonialsColumns">;
    statsColumns: LayoutColumns<"statsColumns">;
    galleryColumns: LayoutColumns<"galleryColumns">;
    journalColumns: LayoutColumns<"journalColumns">;
    faqDefaultOpen: LayoutChoice<"faqDefaultOpen">;
            ...
            {theme.layout.heroStyle === "split" && (
              <Field label="這一段有多高">
                ...
                <p className={hintClass}>
                  ...
                  話改照照片比例撐）；七成螢幕是整段至少佔螢幕高的七成、比一個螢幕短一截、
                  還留得住開頭的份量；跟預設就是整整一個螢幕高。這三檔說的都是「至少多高」，
                  字比這更多時段會跟著長，不會裁字——跟滿版照片版型那格「照片最高佔多少
                  螢幕」用同一把尺量（七成 / 一個螢幕），只是那格是上限、這格是下限。
                  照片直式又寫了三四行字的店維持跟預設。手機是圖上文下堆疊，也照同一把尺：
                  跟著內容就是照片加文字兩塊剛好多高、七成螢幕至少七成、跟預設至少八成；
                  兩塊加起來比這矮時多出來的高度全給文字那段（照片照自己的比例，不會跟文字
                  之間空出一條）
                </p>
              </Field>
            )}
            ...
            {theme.layout.heroStyle === "full-image" && (
              <Field label="照片最高佔多少螢幕">
                ...
                <p className={hintClass}>
                  ...
                  左右分欄版型那格「這一段有多高」是同一把尺，換版型時不用重新換算
                </p>
              </Field>
            )}
            {theme.layout.heroStyle === "full-image" &&
        ...
        {activeTab === "section" && selectedSection && selectedSection !== "hero" && (() => {
          ...
          const presets: {
            ...
            },
            {
              ...
              group: "card",
              label: "整齊格子",
              hint: "卡片加淡底＋一圈細框 + 文字靠左 + 品名兩行（收小加中黑）+ 描述兩行 + 品名與描述行距一起收緊 + 照片正方（每張卡有自己的邊界、同一列下緣切齊；適合欄數調到 3、4 欄的 選物 / 精選）",
              fields: {
                // 這個版型整組話術就是「每張卡有自己的邊界」，可是原本只給淡底那一檔——
                // 那塊底是 currentColor 的 6%，畫在站上預設那種奶油白的段落上，等於在米白
                // 底上再疊一層極淡的灰。一列 3、4 張的小卡照片又是正方形、幾乎貼滿卡片上半，
                // 剩下露出底色的只有下面那三四行字的背景與四周 14px 的內距，那一圈淡到
                // 看不出哪裡是卡片的邊，客人看到的還是「一堆照片跟字自己排成格子」。
                // 改成底＋框那一檔：底收成 4%、外面補一條 22% 的線（見 layout.tsx 那組），
                // 邊界是被線收住的，跟背景多淺無關。順帶把框線粗細 / 深淺 / 樣式那三格也
                // 一起開出來（那三格的條件是「畫面上真的有線」），商家按完還能自己再調。
                // 只影響之後按這顆的商家：preset 是按下去才寫值，已經套過的店照舊。
                cardSurface: "both",
                cardText: "left",
                cardTitleLines: "two",
                cardDescLines: "two",
          ...
          return (
            <PanelSection title="區段樣式">
              ...
              {divider !== "none" && (
                <Field label="分隔線線型">
                  <OptionGrid
                    options={SECTION_LINE_STYLE_OPTIONS}
        ...
        {activeTab === "content" && (
          <PanelSection title="頁尾（Footer）">
            ...
            <Field label="店面資訊小標">
              <input
                type="text"
                value={theme.homepage.footerVisitLabel}
                onChange={(e) =>
